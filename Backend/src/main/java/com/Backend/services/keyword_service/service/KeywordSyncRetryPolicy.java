...
@Component
public class KeywordSyncRetryPolicy {

    public KeywordSyncRetryDecision decide(Throwable error, int nextAttempt) {
        ...
        if (root instanceof WebClientResponseException responseEx) {
            ...
            if (status == HttpStatus.TOO_MANY_REQUESTS) {
                ...
                return new KeywordSyncRetryDecision(true, addJitter(delay), "TMDB_429", message);
            }
            if (status != null && status.is5xxServerError()) {
                Duration delay = cappedBackoff(Duration.ofSeconds(30), nextAttempt, Duration.ofMinutes(15));
                return new KeywordSyncRetryDecision(true, addJitter(delay), "TMDB_5XX", message);
            }
            if (status == HttpStatus.UNAUTHORIZED || status == HttpStatus.FORBIDDEN) {
                return new KeywordSyncRetryDecision(false, Duration.ZERO, "TMDB_AUTH", message);
            }
            if (status == HttpStatus.NOT_FOUND) {
                return new KeywordSyncRetryDecision(false, Duration.ZERO, "TMDB_NOT_FOUND", message);
            }
            return new KeywordSyncRetryDecision(false, Duration.ZERO, "TMDB_CLIENT_" + responseEx.getStatusCode().value(), message);
        }
        ...
        if (root instanceof WebClientRequestException) {
            ...
            return new KeywordSyncRetryDecision(true, addJitter(delay), "TMDB_NETWORK", message);
        }

        if (isTransientDatabaseError(root)) {
            Duration delay = cappedBackoff(Duration.ofSeconds(5), nextAttempt, Duration.ofMinutes(5));
            return new KeywordSyncRetryDecision(true, addJitter(delay), "DB_TRANSIENT", message);
        }

        return new KeywordSyncRetryDecision(false, Duration.ZERO, "UNKNOWN", message);
    }
